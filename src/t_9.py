'''
 * @desc : tensorflow实现 resNet
 * @auth : TYF
 * @date : 2019/8/31 - 15:58

 HighWay NetWork : 相当于修改了每一层的激活函数。
                   传统的激活函数只是对输入x做了非线性变换  y = H(x,w)
                   改进的激活函数保留了一定比例的原始输入   y = H(x,w)T(x,W)+ xC(x,w) 其中T(x,W)+C(x,w)=1
                   例如当T(x,W)和C(x,w)各取0.5，则相当于原始输入x一半激活另一半不激活直接传输到下一层，反向传播
                   的时候让更多的信息直接回流到输入而不需要经过一个非线性变化

 ResNet : 原理与HN很相似，模型随着网络层数加深，中间层的准确率已经趋于饱和了向后则会降低准确率，
          所以再后面加几个恒等映射层(该层的输入等于输出)，增加了网络深度而误差却不会增加，优化了训练过程
          浅层layer(A)的输出为     x
          深层layer(B)的输出为     H(x)
          两层输出的差异即残差为   F(x)
          如果A-B已经趋于饱和则两层输出应该一致(即恒等映射)  H(x) - x = F(x) = 0

          训练的目标是将F(x)逼近与0 实现网络准确率饱和时增加了网络深度而误差却不会增加且优化训练过程的目的
'''
